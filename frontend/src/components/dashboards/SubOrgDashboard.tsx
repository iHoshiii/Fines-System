        ...
        <div>
            <div className="page-header">
                <div className="page-header-left">
                    ...
                    <p>Welcome to the Sub-Organization Dashboard. Manage your organization's fines here.</p>
                </div>
                    ...
                    <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 600 }}>
                        ...
                        <div className="modal-body" style={{ maxHeight: '60vh', overflowY: 'auto' }}>
                            <div className="flex-col gap-sm">
                                {allFines.filter(f => f.student_id === selectedStudent.id).map(fine => (
                                    <div key={fine.id} className="flex-between p-sm card" style={{ background: 'var(--color-bg-alt)' }}>
                                        <div><p style={{ fontWeight: 600 }}>{fine.description}</p><p className="text-xs text-muted">{format(new Date(fine.created_at), 'MMM d, yyyy')}</p></div>
                                        <div style={{ textAlign: 'right' }}><p style={{ fontWeight: 700, color: fine.status === 'unpaid' ? 'var(--color-danger)' : 'var(--color-success)' }}>₱{Number(fine.amount).toFixed(2)}</p><span className={`badge badge-${fine.status}`} style={{ fontSize: 10 }}>{fine.status.toUpperCase()}</span></div>
                                    </div>
                                ))}
                            </div>
                        </div>
